...
# fileWay = './6 задача_пример расчета.xlsx'
def readExcel(fileWay):
    ...
    location = data['Местоположение'][0].rstrip(',').split()
    ...
    print(metro)
    metroDistance = f'{data["Удаленность от станции метро, мин. пешком"][0]}п'
    floor = data["Этаж расположения"][0]
    floors = data["Этажность дома"][0]
    area = data["Площадь квартиры, кв.м"][0]
    isLastFloor = 0
    if (floor == floors):
        isLastFloor = 1
    balcon = '-'
    if (data["Наличие балкона/лоджии"][0].lower() == "да"):
        balcon = 'Б'
    wc = 'Р'
    houseType = '-'
    if (data["Наличие балкона/лоджии"][0].lower() == "панель"):
        houseType = 'П'
    if (data["Наличие балкона/лоджии"][0].lower() == "монолит"):
        houseType = 'М'
    if (data["Наличие балкона/лоджии"][0].lower() == "кирпич"):
        houseType = 'К'
